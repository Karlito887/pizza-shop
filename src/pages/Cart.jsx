...
import { deleteAllPizzas, deletePizzasGroup, addPizzaToCart, minusPizza } from '../redux/actions/cart'

function Cart() {
    const { totalCount, totalPrice, items } = useSelector(({ cart }) => ({
    ...
    const deletePizzaById = (id) => {
        return dispatch(deletePizzasGroup(id))
    }

    const deletePizza = () => {
        return dispatch(deleteAllPizzas())
    }

    const onClickAddPizza = (obj) => {
        return dispatch(addPizzaToCart(obj))
    }
    ...
    const totalCountCartPizzas = Object.keys(items).map((key) => {
        return items[key].length
        })
    const totalPriceCartPizzas = Object.keys(items).map((key) => {
        return items[key].reduce((prev, item) => {
 ...
 Корзина</h2>
                    <div className="cart__clear" onClick={deletePizza}>
                ...
                <div className="content__items">
                    {addedPizzas.length > 0 && addedPizzas.map((obj, index) => {
                        return <CartPizzaBlock key={obj + index} {...obj} 
                        totalCountCartPizzas={totalCountCartPizzas[index]} 
                        totalPriceCartPizzas={totalPriceCartPizzas[index]} 
